...
func New(tag string, level LogLevelType, flag int, out io.Writer) *Loger {
	l := &Loger{tag: tag, level: level, flag: flag, out: out}
	l.isDiscard = 0
	if out == io.Discard {
		l.isDiscard = 1
	}
...
func (l *Loger) SetWriter(w io.Writer) {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.out = w
	l.isDiscard = 0
...
func (l *Loger) Writer() io.Writer {
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.out
}

func (l *Loger) SetTag(tag string) {	
	l.mu.Lock()
	defer l.mu.Unlock()
	l.tag = tag
}

func (l *Loger) Tag() string {
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.tag
}

func (l *Loger) SetCallDepth(calldepth int) {	
	l.mu.Lock()
	defer l.mu.Unlock()
	l.calldepth = calldepth
}

func (l *Loger) CallDepth() int {
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.calldepth
}

func (l *Loger) SetLevel(level LogLevelType) {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.level = level
}

func (l *Loger) Level() LogLevelType {
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.level
}
...
func (l *Loger) formatHeader(buf *[]byte, t time.Time, file string, line int, level LogLevelType) {		
	if l.flag&LPreTag != 0{
...
func (l *Loger) Output(calldepth int, level LogLevelType, s string) error {
	var file string
	var line int 
	now := time.Now()
	l.mu.Lock()
...
func (l *Loger) Printf(format string, v ...any) {
...
func (l *Loger) Fatalf(format string, v ...any) {
	l.Output(2 + l.calldepth, l.level, fmt.Sprintf(format + "\n", v...))
	os.Exit(1)
}

func (l *Loger) Fatal(v ...any) {
	l.Output(2 + l.calldepth, l.level, fmt.Sprint(v...))
	os.Exit(1)
}

func (l *Loger) Fatalln(v ...any) {
	l.Output(2 + l.calldepth, l.level, fmt.Sprintln(v...))
	os.Exit(1)
}

func (l *Loger) Panicf(format string, v ...any) {
	s := fmt.Sprintf(format + "\n", v...)
	l.Output(2 + l.calldepth, l.level, s)
	panic(s)
}

func (l *Loger) Panic(v ...any) {
	s := fmt.Sprint(v...)
	l.Output(2 + l.calldepth, l.level, s)
	panic(s)
}

func (l *Loger) Panicln(v ...any) {
	s := fmt.Sprintln(v...)
	l.Output(2 + l.calldepth, l.level, s)	
	panic(s)
}

func (l *Loger) Debugf(format string, v ...any) {
	printfByLevel(l, LogDebug, format,  v...)
}

func (l *Loger) Debug(v ...any) {
	printByLevel(l, LogDebug, v...)
}

func (l *Loger) Debugln(v ...any) {
	printlnByLevel(l, LogDebug, v...)
}

func (l *Loger) Infof(format string, v ...any) {
	printfByLevel(l, LogInfo, format, v...)
}

func (l *Loger) Info(v ...any) {
	printByLevel(l, LogInfo, v...)
}

func (l *Loger) Infoln(v ...any) {
	printlnByLevel(l, LogInfo, v...)
}

func (l *Loger) Warnf(format string, v ...any) {
	printfByLevel(l, LogWarn, format, v...)
}

func (l *Loger) Warn(v ...any) {
	printByLevel(l, LogWarn, v...)
}

func (l *Loger) Warnln(v ...any) {
	printlnByLevel(l, LogWarn, v...)
}

func (l *Loger) Errorf(format string, v ...any) {
	printfByLevel(l, LogError, format, v...)
}

func (l *Loger) Error(v ...any) {
	printByLevel(l, LogError, v...)
}

func (l *Loger) Errorln(v ...any) {
	printlnByLevel(l, LogError, v...)
}

func (l *Loger) Criticalf(format string, v ...any) {
	printfByLevel(l, LogCritical, format, v...)
}

func (l *Loger) Critical(v ...any) {
	printByLevel(l, LogCritical, v...)
}

func (l *Loger) Criticalln(v ...any) {
	printlnByLevel(l, LogCritical, v...)
}

func (l *Loger) PrintfIgnoreLevel(level LogLevelType, format string, v ...any) {
	if atomic.LoadInt32(&l.isDiscard) != 0 {
		return 
	}
...
func (l *Loger) PrintIgnoreLevel(level LogLevelType, v ...any) {	
	if atomic.LoadInt32(&l.isDiscard) != 0 {
		return 
	}
...
func (l *Loger) PrintlnIgnoreLevel(level LogLevelType, v ...any) {
	if atomic.LoadInt32(&l.isDiscard) != 0 {
		return 
	}
...
func printfByLevel(l *Loger, level LogLevelType, format string, v ...any) {
	if atomic.LoadInt32(&l.isDiscard) != 0 {
		return 
	}
...
func printByLevel(l *Loger, level LogLevelType, v ...any) {
	if atomic.LoadInt32(&l.isDiscard) != 0 {
		return 
	}
	if atomic.LoadInt32(&l.level) <= level {
...
func printlnByLevel(l *Loger, level LogLevelType, v ...any) {
	if atomic.LoadInt32(&l.isDiscard) != 0 {
		return 
	}
	if atomic.LoadInt32(&l.level) <= level {
